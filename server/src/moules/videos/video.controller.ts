...
import { createVideo, finadAllVideos, findVideo } from "./video.service";
...
export async function uploadVideoHanler(req: Request, res: Response) {
  //use busboy to go save chunks of video insted of all
  ...
  bb.on('file', async (_, file, info) => {
    ...
    const filePath = getPath({ videoId: video.videoId, extension: extension });
...
export async function findAllVideosHandler(_:Request,res:Response){
  const videos = await finadAllVideos();
...
export async function streamVideoHandler(req: Request, res: Response) {
  ...
  const chunkEnd = Math.min(
    ...
    CHUNK_SIZE_IN_BYTES,
    fileSizeInBytes - 1);
  const contentLen = chunkEnd - chunkStart + 1;
  const headers = {
